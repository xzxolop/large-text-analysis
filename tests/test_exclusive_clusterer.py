...
def sample_tfidf_data():
    ...
    # 5 предложений, 6 слов
    # Предложения:
    # 0: "machine learning is great"
    # 1: "deep learning is awesome"
    # 2: "python is great for data"
    # 3: "data science uses machine learning"
    # 4: "neural networks are deep"
    
    tfidf_matrix = csr_matrix([
        [0.5, 0.5, 0.3, 0.0, 0.0, 0.0],  # doc 0: machine, learning, great
        [0.0, 0.6, 0.4, 0.0, 0.5, 0.0],  # doc 1: learning, awesome, deep
        [0.0, 0.0, 0.4, 0.5, 0.0, 0.4],  # doc 2: great, python, data
        [0.4, 0.4, 0.0, 0.5, 0.0, 0.0],  # doc 3: machine, learning, data
        [0.0, 0.0, 0.0, 0.0, 0.6, 0.5],  # doc 4: deep, networks
    ])
...
class TestCluster:
    ...
    def test_cluster_all_docs_assigned(self, clusterer):
        ...
        # Все 5 предложений должны быть назначены
    ...
    def test_cluster_different_metrics(self, sample_tfidf_data):
        """Разные метрики должны давать разные результаты."""
        tfidf_matrix, feature_names, word_freqs = sample_tfidf_data
        clusterer = ExclusiveClusterer(tfidf_matrix, feature_names, word_freqs)
        
        clusters_tfidf = clusterer.cluster(metric="tfidf")
        clusters_logfreq = clusterer.cluster(metric="tfidf_logfreq")
        clusters_div = clusterer.cluster(metric="tfidf_div_logfreq")
        
        # Результаты могут отличаться
        # (не обязательно всегда разные, но метрики должны работать)
        assert len(clusters_tfidf) > 0
        assert len(clusters_logfreq) > 0
        assert len(clusters_div) > 0

    def test_cluster_invalid_metric(self, clusterer):
        """Неверная метрика должна вызвать ошибку."""
        with pytest.raises(ValueError, match="Unknown metric"):
            clusterer.cluster(metric="invalid_metric")


class TestClusterWithScores:
    """Тесты метода cluster_with_scores()."""

    def test_returns_tuple(self, clusterer):
        """Метод должен возвращать кортеж из двух элементов."""
        result = clusterer.cluster_with_scores(n=5)
        assert isinstance(result, tuple)
        assert len(result) == 2

    def test_clusters_and_scores(self, clusterer):
        """Кластеры и скоры должны соответствовать друг другу."""
        clusters, scores = clusterer.cluster_with_scores(n=5)
        
        assert len(scores) == 5
        assert all(s >= 0 for s in scores)

    def test_scores_match_metric(self, clusterer):
        """Скоры должны соответствовать выбранной метрике."""
        clusters_tfidf, scores_tfidf = clusterer.cluster_with_scores(metric="tfidf")
        clusters_logfreq, scores_logfreq = clusterer.cluster_with_scores(metric="tfidf_logfreq")
        
        # Скоры должны отличаться для разных метрик
        assert len(scores_tfidf) == len(scores_logfreq)


class TestGetClusterStats:
    """Тесты метода get_cluster_stats()."""

    def test_stats_returns_dict(self, clusterer):
        """Метод должен возвращать словарь со статистикой."""
        clusters = clusterer.cluster(n=5)
        ...
        for word, stat in stats.items():
            ...
            assert "indices" in stat

    def test_stats_sorted_by_size(self, clusterer):
        """Статистика должна быть отсортирована по размеру кластера."""
        clusters = clusterer.cluster(n=5)
        stats = clusterer.get_cluster_stats(clusters)
    ...
    def test_stats_percentages_sum_to_100(self, clusterer):
...
class TestGetTopClusters:
    ...
    def test_min_cluster_size_filters(self, clusterer):
        ...
        # Все кластеры в отфильтрованном результате должны иметь размер >= 2
...
class TestPerformance:
    ...
    def test_large_matrix(self):
        ...
        # Создаём случайную разреженную матрицу
